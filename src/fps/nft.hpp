...
template <class Mint>
V<Mint> cyconv(V<Mint> a, V<Mint> b, int k) {
  int z = 1;
  while (z < k) z *= 2;
  a.resize(z);
  b.resize(z);
...
V<Mint> multiply(V<Mint> a, V<Mint> b) {
  ...
  return cyconv(move(a), move(b), n + m - 1);
}
// multieval, bostan_mori, composition
template <class Mint>
V<Mint> tr_multiply(V<Mint> a, V<Mint> b) {
  int n = a.size(), m = b.size();
  reverse(b.begin(), b.end());
  b = cyconv(move(a), move(b), m);
  reverse(b.begin(), b.end());
  b.resize(m - n + 1);
  return b;
}